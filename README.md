...
2. **Desempeño defensivo univariado según Posición:** Los jugadores cuya posición en cancha está más cerca del arco rival (`FW`, `MF`), ¿interceptaron una menor cantidad de pases (`interceptions`) que el resto (`DF`)? *(Análisis numérico vs. categórico: `interceptions` según `position`)*.
